#include "flash.h"
#include "stm32f1xx.h"
void Flash_EraseAll(uint32_t address)
{
   HAL_FLASH_Unlock();
	 FLASH_EraseInitTypeDef EraseInitStruct1;
	 EraseInitStruct1.Banks =1;
	 EraseInitStruct1.NbPages = 1;
	 EraseInitStruct1.PageAddress = address;
	 EraseInitStruct1.TypeErase = FLASH_TYPEERASE_MASSERASE;
	 uint32_t pageerr1;
	 HAL_FLASHEx_Erase(&EraseInitStruct1, &pageerr1);
	 HAL_FLASH_Lock();
}
void Flash_Erase(uint32_t address)// XOA 1 DIA CHI
{
   HAL_FLASH_Unlock();
